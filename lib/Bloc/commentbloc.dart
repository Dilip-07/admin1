...
class CommentBloc extends ChangeNotifier {
  ...
  Future saveNewComment(
      String timestamp, String review, String collectionName) async {
    String _uid = 'admin'; // admin uid
    String _adminName = 'Admin'; //admin name
    String _imageUrl =
        'https://img.icons8.com/bubbles/2x/admin-settings-male.png'; //admin icon url
  ...
  Future deleteComment(String timestamp, String uid, String timestamp2,
  ...
  Future<int> getTotalPlaceReviws(String timestamp) async {
    final DocumentReference ref =
        firestore.collection('Tournaments').doc(timestamp);
  ...
  Future increaseCommmentsCount(String timestamp) async {
    await getTotalPlaceReviws(timestamp).then((int documentCount) async {
      await firestore
          .collection('Tournaments')
          .doc(timestamp)
          .update({'comments count': documentCount + 1});
    });
  }
