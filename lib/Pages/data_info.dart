...
class _DataInfoPageState extends State<DataInfoPage> {
  ...
  void initState() {
    ...
    tournament =
        context.read<AdminBloc>().getTotalDocuments('tournaments_count');
    games = context.read<AdminBloc>().getTotalDocuments('games_count');
  }
          ...
          Row(
            ...
            children: <Widget>[
              ...
              FutureBuilder(
                future: tournament,
                builder: (BuildContext context, AsyncSnapshot snap) {
                  if (!snap.hasData) return card('TOTAL TOURNAMENTS', 0);
                  if (snap.hasError) return card('TOTAL TOURNAMENTS', 0);
                  return card('TOTAL Tournaments', snap.data);
                },
              ),
              SizedBox(
                width: 20,
              ...
              FutureBuilder(
                future: games,
                builder: (BuildContext context, AsyncSnapshot snap) {
                  if (!snap.hasData) return card('TOTAL GAMES', 0);
                  if (snap.hasError) return card('TOTAL GAMES', 0);
                  return card('TOTAL Games', snap.data);
                },
              ),
            ],
          ),
